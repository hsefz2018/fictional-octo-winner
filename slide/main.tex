...
\begin{itemize}
    \item Algorithm 3 \\
        ...
        $$
            Ans = -\sum_{|S| = n} f(S) + \sum_{|S| = n - 1} f(S) - \sum_{|S| = n - 2} f(S) + \dots \sum_{|S| = 0} f(S)
        $$
\end{itemize}
...
感谢出题人 RYZ 完成的出色任务。\\
感谢队长 wangyurzee 和数学大神 DZJ 在本烧饼推公式方面提供的滋瓷。\\
感谢各大神ben（字体的锅） blogger 的部分题解。\\
感谢 LYDSY Online Judge 使得确认这里内容的正确性成为可能。\\
感谢特派员 WSG 老师 \& 各种老师对本次比赛的大力滋瓷。\\
\small{然而还是没有进队}
　\\　\\
